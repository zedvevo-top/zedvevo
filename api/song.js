...
export default async function handler(req, res) {
  ...
  if (targetId) {
    try {
      ...
      const response = await fetch(`${supabaseUrl}/rest/v1/songs?${query}&select=*&limit=1`, {
        headers: {
          'apikey': supabaseKey,
      ...
      console.error('API song metadata fetch error:', e);
    }
  }
  ...
  if (song) {
    const artistName = song.artist_name || song.artist || 'ZedVevo Artist';
    const pageTitle = `${song.title || 'Song'} by ${artistName} — ZedVevo`;
    const pageDescription = `Stream and download "${song.title || 'Song'}" by ${artistName} on ZedVevo. ${song.album ? 'Album: ' + song.album : ''}`;
    const rawCover = song.cover_url || song.thumbnail_url || '';
    ...
    const currentUrl = `${baseUrl}/song/${encodeURIComponent(targetId)}`;
