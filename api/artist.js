...
export default async function handler(req, res) {
  ...
  if (targetId) {
    try {
      ...
      const query = isUuid
        ...
        : `or=(id.eq.${encodeURIComponent(targetId)},stage_name.ilike.${encodeURIComponent(targetId)})`;

      const response = await fetch(`${supabaseUrl}/rest/v1/artists?${query}&select=*&limit=1`, {
        headers: {
          'apikey': supabaseKey,
      ...
      console.error('API artist metadata fetch error:', e);
    }
  }
  ...
  if (artist) {
    const displayName = artist.stage_name || artist.name || 'Zambian Artist';
    const pageTitle = `${displayName} — Stream Music & Videos on ZedVevo`;
    const pageDescription = artist.bio || `Listen to top songs, stream albums, and watch official music videos by ${displayName} on ZedVevo. Home of Zambian entertainment.`;
    const coverUrl = artist.avatar_url || artist.cover_image_url || 'https://zedvevo.xyz/og-image.png';
    ...
    const currentUrl = `${baseUrl}/artist/${encodeURIComponent(targetId)}`;
