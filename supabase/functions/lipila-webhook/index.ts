...
serve(async (req) => {
  ...
  if (newStatus === 'completed' && payment.payment_type === 'vote') {
    ...
    if (nominee_id && category_id) {
      // Check if vote already created for this payment to prevent duplicates
      ...
      // Automatically recalculate and sync nominee total_votes
      const { data: sumData } = await supabase
        .from('votes')
        .select('vote_count')
        .eq('nominee_id', nominee_id)
        .eq('payment_status', 'successful');
      const totalVotes = (sumData || []).reduce((acc: number, v: any) => acc + (v.vote_count || 1), 0);
      await supabase.from('nominees').update({ total_votes: totalVotes }).eq('id', nominee_id);
      console.log('[webhook] nominee total_votes updated to:', totalVotes);
    }
  }
